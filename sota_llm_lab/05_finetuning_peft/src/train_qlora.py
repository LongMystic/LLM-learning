"""
05.3 – Train QLoRA (4-bit quantization + LoRA) trên base model (TinyLlama).
Giảm VRAM/RAM so với 05.2; dùng dataset từ prepare_data.py.
GPU: cần pip install bitsandbytes. CPU/Windows: tự fallback LoRA (không quantize), lưu vào qlora_run.
"""
import torch
...
from prepare_data import prepare_dataset, DEFAULT_MODEL_NAME

# Cấu hình
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "experiments" / "qlora_run"
LORA_R = 8
LORA_ALPHA = 16
...
SAVE_STEPS = 50
MAX_STEPS = -1
USE_4BIT = True  # True = 4-bit (QLoRA) khi có GPU + bitsandbytes
...
def main():
    ...
    model = _load_model_qlora(use_cuda)
